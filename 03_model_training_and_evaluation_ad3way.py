...
def compute_meta_shap(meta_model, x_te_meta: np.ndarray,
                      feature_names: list, prefix: str = 'meta_shap') -> dict:
    """
    ...
    try:
        ...
        return {f'{prefix}_{fn}': float(mean_abs[i])
                for i, fn in enumerate(feature_names)}
    except Exception:
        return {f'{prefix}_{fn}': np.nan for fn in feature_names}
...
def main():
    ...
    for h in H_LIST:
        ...
        for i in inner:
            ...
            if SAVE_STEP_RECORDS:
                for j in range(len(y_te)):
                    ...
                    if SAVE_META_SHAP and meta_m2 is not None and te_meta2_X is not None and np.isfinite(te_meta2_X[j:j+1]).all():
                        x_for_shap = te_meta2_X[j:j+1]
                        shap_dict2 = compute_meta_shap(
                            meta_m2, x_for_shap, meta2_feat_names, prefix='meta2_shap')
                        rec.update(shap_dict2)
                        # 兼容旧可视化链路：保留原 meta_shap_* 别名，仍指向二路 Meta
                        rec.update({
                            k.replace('meta2_shap_', 'meta_shap_'): v
                            for k, v in shap_dict2.items()
                        })
                    if SAVE_META_SHAP and meta_m3 is not None and te_meta3_X is not None and np.isfinite(te_meta3_X[j:j+1]).all():
                        x_for_shap3 = te_meta3_X[j:j+1]
                        shap_dict3 = compute_meta_shap(
                            meta_m3, x_for_shap3, meta3_feat_names, prefix='meta3_shap')
                        rec.update(shap_dict3)
        ...
        if RUN_TABPFN_SHAP and len(tabpfn_shap_pool) >= TABPFN_SHAP_N:
            ...
            if not df_shap.empty:
                ...
                shap_path = os.path.join(EVAL_DIR, f'tabpfn_shap_{OUTPUT_TAG}_h{h}.csv')
    ...
    save_path = os.path.join(EVAL_DIR, f'Final_ShortHorizon_v2_{OUTPUT_TAG}.xlsx')
    ...
    print(f"   Step_Records  : 每步日期/预测/市场状态/Meta2+Meta3 SHAP（机制分析用）")
    if RUN_TABPFN_SHAP:
        print(f"   TabPFN-SHAP   : tabpfn_shap_{OUTPUT_TAG}_h{{h}}.csv（各视界独立文件）")
